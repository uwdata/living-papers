...
export default async function(ast, context, options) {
  ...
  const cssSourceMapPath = path.join(outputDir, cssFile + '.map');
  ...
  await writeFile(path.join(styleDir, '_theme.scss'), `// Autogenerated by build script. Do not modify!\n@import "themes/${theme}";`);

  // bundle style sheets
  let { css, sourceMap } = await bundleCSS(path.join(styleDir, 'style.scss'), rollupOptions.minify);
  css += `/*# sourceMappingURL=${cssFile}.map */`;
  ...
  await Promise.all([
    ...
    ...(selfContained ? [] : [writeFile(cssPath, css), writeFile(cssSourceMapPath, sourceMap)])
  ]);
...
async function bundleCSS(scssFile, minify = true) {
  let { css, sourceMap } = await sass.compile(scssFile, {
    sourceMap: true,
