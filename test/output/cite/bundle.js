...
class CiteBib extends s {
  render() {
    return $`<h1>References</h1><slot></slot>`;
  }
}

class CiteRef extends s {
  static get styles() {
    return r$2`
      .citation {
  ...
  render() {
    ...
    return $`<span class="citation" title=${title}><slot name="prefix"></slot>${body}<slot name="suffix"></slot></span>`;
  }
}
...
window.customElements.define('cite-bib', CiteBib);
window.customElements.define('cite-ref', CiteRef);
window.customElements.define('cite-list', CiteList);
