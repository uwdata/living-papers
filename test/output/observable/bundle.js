...
function resolver$1(name, version) {
  return path => `https://cdn.jsdelivr.net/npm/${name}@${version}/${path}`;
}

function style$1(href, doc = globalThis.document) {
...
function loadDependencies(object) {
  ...
  schema.forEach(({ name, version, main, css }) => {
    const resolve = resolver$1(name, version);
    ...
    if (css && !cache.has(cssURI)) {
      ...
      load.css.push(style$1(cssURI, object.ownerDocument));
    }
  });
...
window.addEventListener('DOMContentLoaded', () => {
  hydrate(
    ...
    module,
    {"_id":2,"attrs":[["2","code",0]],"events":[["1","click",0]]}
  );
});
